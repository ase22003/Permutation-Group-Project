...
function find_path(generators::SetOfPermutations, target::Permutation)
	i::UInt16 = 0
	function search(node::Permutation, path::SetOfPermutations)::SetOfPermutations; i+=1
		if node == target
			return path
		end
...
function find_path(generators::SetOfPermutations) #GPT
	...
	new_nodes::SetOfPermutations = SetOfPermutations([])
	...
	function backtrack(node::Permutation)::Vector
