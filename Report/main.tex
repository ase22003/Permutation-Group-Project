...
\begin{lemma}\label{lmconj}
	...
	\(\alphaset\cap\betaset=\{c\}\). Then the cycle of \(\conjl\alpha\beta\) wherein \(c\) occurs has the form \(\left(\alpha\inv(c),\beta(c),\beta^2(c),\cdots,\beta\inv(c)\right)\).
\end{lemma}
...
\begin{proof}
	...
	\begin{equation}\label{conj}
		\conjl \alpha \beta (x) = \left\{
			...
			\right.
	\end{equation}
	which, in cycle notation, is written \(\left(\alpha\inv(c),\beta(c),\beta^2(c),\cdots,\beta\inv(c)\right)\cdots\).
\end{proof}
...
\begin{lemma}\label{conj_string}
	...
	Let \(\alphaset\cap\betaset = \set{\spann {c_1} c_n },\, \alpha\neq\beta\) for \(n\geq 2\) where \(\alpha(c_m)=\alpha(c_{m+1})\) for \(m\in\set{\spann 1 n-1}\) and \(\alpha^k(c_1) = \beta^k(c_1)\) for \(k\in\set{\spann 0 k-1}\). Then \(\conjl \alpha \beta = \perm{\alpha\inv(c_1), \spann {c_1} c_{n-1},\spann {\beta(c_{n})} \beta\inv(c_1)}\).
\end{lemma}
	...
	\begin{align}
		\comml \alpha \beta (x) &= \left\{
				\begin{array}{cl}
					...
					\beta\inv\left(\beta(x)\right) & : \ x\in\set{\spann {\beta(c_n)} \beta^{-2}(c_1)} \cup \set{\spann {c_1} c_{n-2}} \\
					\beta\inv(\beta(c_n)) & : \ x=c_{n-1} \\
					\beta\inv\left(\alpha\inv(c_1)\right) & : \ x=\beta\inv(c_1)
				\end{array}
			\right. \\
		&= \left\{
				\begin{array}{cl}
