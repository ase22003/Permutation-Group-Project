...
\begin{proof}
	...
	A permutation always commutes with itself, so if \(\alpha_c=\beta_c\) for some \(c\in\alphaset\cap\betaset\) -- where \(\alpha_c\) and \(\beta_c\) are the cycles of \(\alpha\) and \(\beta\) wherein \(c\) occurs -- then \(\alpha_c\circ\beta_c=\beta_c\circ\alpha\). If each cycle in \(\alpha\) and \(\beta\) commute, so do they.
